...
1. Установите [Arduino IDE](https://www.arduino.cc/en/software) (версия 2.0+)
...
Через Arduino IDE:
Выберите плату: Инструменты → Плата → ESP32 Arduino → ESP32C3 Dev Module
...
Подключите ESP32-C3 через USB-кабель

Выберите порт: Инструменты → Порт → COMx (ESP32-C3)

Нажмите Загрузить (Стрелка вправо)

При ошибке загрузки: удерживайте кнопку BOOT на плате во время нажатия кнопки "Загрузить"
